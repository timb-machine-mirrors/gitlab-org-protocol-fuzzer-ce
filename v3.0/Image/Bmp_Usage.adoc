include::Bmp_DataSheet.txt[]

Configuration
-------------

.Target Configuration
Any number of image viewing programs may be targeted with the BMP file format. Example: targets for fuzzing BMP are "feh" on Linux and "mspaint.exe" on Windows. The target program can be set in the Bmp.xml.config file by defining the Target program for each operating system.
In the Bmp.xml.config pit file, you can set the target program that you are fuzzing, such as feh or mspaint.exe. Normally you would set different target programs for different operating systems.
...
Target:: The program that will open the fuzzed Bmp files
...
<PitDefines>
  ...
  <All>
    <Define key="Seed" value="bmp_sample_01.bmp"/>
    <Define key="Strategy"  value="Sequential"/>
    <Define key="LoggerPath"  value="./logs/bmp/"/>
