...
def init_seq(ctx):
    # print "init running"
    seed = (int(ctx.parent.parent.parent.context.test.strategy.Iteration)
+ int(ctx.parent.parent.parent.context.test.strategy.Seed))
    # print "making CRC"
...
def set_next_seq(ctx):
    ret = set_to_store(ctx,
NextSequenceNumber=int(ctx.dataModel.find('SequenceNumber').InternalValue.ToString())+(len(ctx.dataModel.find('TcpPayload').Value.Value)
or 1))
...
def sync_from_store(ctx):
    ctx.dataModel.find('SrcPort').DefaultValue =
Peach.Core.Variant(get_store_val(ctx, "SrcPort"))
    if bool(int(ctx.dataModel.find('ACK').InternalValue)):
        set_default_from_store(ctx, "AcknowledgmentNumber")
    ctx.dataModel.find("SequenceNumber").DefaultValue =
Peach.Core.Variant(get_store_val(ctx, "NextSequenceNumber"))
    ctx.dataModel.Value


def store_next_acknum(ctx):
    ret = set_to_store(ctx,
AcknowledgmentNumber=int(ctx.dataModel.find('SequenceNumber').InternalValue.ToString())+(len(ctx.dataModel.find('TcpPayload').InternalValue.ToString())
or 1))
...
def set_default_from_store(ctx, name):
    ctx.dataModel.find(name).DefaultValue =
Peach.Core.Variant(get_store_val(ctx, name))
    ctx.dataModel.Value #this shouldn't need to be called in future
...
def chk_if_ack_for_me(ctx):
    #print "AckNum ",
int(ctx.parent.actions[0].dataModel.find('AcknowledgmentNumber').InternalValue.ToString())
    #print "Expected AckNum ", get_store_val(ctx, 'NextSequenceNumber')
    ret = int(ctx.parent.actions[0].dataModel.find('AcknowledgmentNumber').InternalValue.ToString())
== (get_store_val(ctx, 'NextSequenceNumber')) and
bool(int(ctx.parent.actions[0].dataModel.find('ACK').InternalValue))
    return ret


def set_timestamp(ctx):
    ctx.dataModel.find('TimestampValue').DefaultValue =
Peach.Core.Variant(get_cur_timestamp())
    ctx.dataModel.Value
    return True
