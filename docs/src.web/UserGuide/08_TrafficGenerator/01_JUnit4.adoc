...
The JUnit 4 integration is an extension to JUnit 4 to create a Peach Suite of test cases.
...
It is not directly callable as it expects session setup to have already been performed.
...
These variables are normally set by the CI runner automatically:

PEACH_API:: URL to {product} API. Ex. http://10.0.0.1
PEACH_API_TOKEN:: API Token for {product}
PEACH_SESSIONID:: ID for current testing session. Returned from /api/sessions API.
