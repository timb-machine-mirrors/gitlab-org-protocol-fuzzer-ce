...
Checks implement the testing logic used by {product}.  Checks can be
...
Endpoint::
+
Endpoints are different hosts that identified during testing.
...
might not actually be a real resources.
+
The types of checks that operate at this scope would include
checking for sensitive file exposure.  Tests that do not
...
PUT, etc. 
Testing performed at the operation level is indipendent of
...
the HTTP request.
This includes path segments, querystring parameter,
