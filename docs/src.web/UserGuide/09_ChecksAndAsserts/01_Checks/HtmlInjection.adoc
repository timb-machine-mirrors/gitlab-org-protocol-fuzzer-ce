...
This check injections various HTML snippets that demonstraight a XSS issue.  
The response is then analyzed to determine if the XSS injection has been included in any
field or response body identified as contianing HTML.
...
NOTE: This check has limited ability to determine if XSS issues exist in your frontend application.
The detection process is limited to analysis of API results.  
To reduce false positives failures only occur when the field or response is known to by HTML.  
For fields, the field must contain
other HTML elements.  
For response bodies the content type must be text/html.
This is not a substitue for testing your frontend application.
