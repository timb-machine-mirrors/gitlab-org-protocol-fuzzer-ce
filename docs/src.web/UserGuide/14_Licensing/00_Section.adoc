...
with a device id.
An instances device id can be found in the Licensing section of
the Settings page.
+
...
The easiest method to determin the correct device id assumes the activated 
instance is still running and available.

. Access {product} in the normal mannor
...
id's of all available instances to determin which device id is not in active
use.
...
. Verify device id is no longer listed
...
aquire a new license and re-activate the instance.  If your seeing the
