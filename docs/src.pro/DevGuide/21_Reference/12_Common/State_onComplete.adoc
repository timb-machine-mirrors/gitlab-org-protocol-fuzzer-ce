...
The _onComplete_ attribute is a python scripting statement that is evaluated prior to performing the actions in a xref:State[State] model. 
This statement can be used to increment a counter or to perform other functions defined in separate files included with the xref:Import[Import] element.
