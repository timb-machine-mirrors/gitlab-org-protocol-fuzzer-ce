...
The _onStart_ attribute is a python scripting statement that is evaluated prior to preforming the actions in a xref:State[State] model.
The scripting statement can be used to increment a counter or preform other functions defined in separate files included with the xref:Import[Import] element.

TIP:  _xref:State_onComplete[onComplete]_ is the sibling attribute.

.Keeping State
****
State bags are defined as _Dictionary<string, object>_ in C#. If you use one of two state bags exposed through the RunContext instance, you can store state during the lifetime of the current iteration or the fuzzing session.

A full example of using the iteration state bag is provided in the below examples.

_Using the Session State:_

This state stored in this state bag will last across iterations for the full fuzzing session.
...
==========================
This example will print a message when the _onStart_ statement is run.
