...
The _Base64DecodeTransformer_ will Base64 decode the value in the parent xref:DataModel[DataModel]. The xref:Transformer[Transformer] can bidirectionally encode data as well as decode data.
...
xref:Action_output[output]:: On output this transfomer will Base64 Decode this outgoing data.

=== Examples

.Decoding Value To and From File Example
==========================
This example uses the Base64Encode transformer on an inline value, writes the transformed value to a file, and reads the value from the file to the console.
