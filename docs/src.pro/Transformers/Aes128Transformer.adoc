...
The _Aes128Transformer_ will transform the parent xref:DataModel[DataModel] by encrypting the data using AES (Rijndael) with the provided Key and Initialization Vector. The xref:Transformer[Transformer] can bidirectionally encrypt and decrypt data.
...
==========================
This example uses the Aes128 transformer on an inline value, writes the transformed value to a file, and reads the value from the file to the console. 
