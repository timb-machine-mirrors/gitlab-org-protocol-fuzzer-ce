...
The _UrlEncodeTransformer_ will produce a url encoded string of the provided value in the parent xref:DataModel[DataModel].  
...
==========================
This example uses the UrlEncode Tranform on an inline value, writes the transformed value to a file, and reads the value from the file to the console. 
