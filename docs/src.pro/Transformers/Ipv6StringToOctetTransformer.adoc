...
The _Ipv6StringToOctetTransformer_ will produce the octet bytes from the string representation of an Internet Protocol version 6 address.
...
xref:Action_input[input]:: On input this transformer will convert bytes into an IP address string representation.
xref:Action_output[output]:: On output this transformer will convert an IP address string representation into bytes.

=== Examples

.Simple Encode Example
==========================
This example uses the Ipv6StringToOctet transformer on an inline value, writes the transformed value to a file, and reads the value from the file to the console.
