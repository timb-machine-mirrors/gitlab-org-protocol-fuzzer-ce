...
Peach allows modeling of relationships in the data.  Relationships are things like "X is the size of Y", "X is the count of Y", or "X is the offset (in bytes) of Y".
...
This example is also true for multi-byte characters such as wchar.  

In future versions of Peach either this will change or a new relation of type length will be included so as to better support UTF-8 and other Unicode encodings.
...
Peach has two variables, +self+ and +size+, that we use in these expressions.  Self is a reference to the Number element and size is an integer. 

Expression gets and sets should be each others mathematical inverse. Gets are applied during the cracking process and Sets are applied during the publishing process.
...
Peach supports the concept of relative offsets.  A relative offset is from the data element the relation is attached to.  

Consider the following example.  When determining the offset of _StringData_ Peach will add/subtract the position of _OffsetToString_ to it's value as needed to determine the correct offset.
...
Peach also supports offsets that are relative to another element.  This is used when an element contains the offset to another element from the start of a structure.  
