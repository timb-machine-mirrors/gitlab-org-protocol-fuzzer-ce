...
This example shows the Syslog monitor listening on the default port for incoming messages. When a fault occurs all messages will be saved.
...
This example shows the Syslog monitor listening on the default port for incoming messages. The FaultRegex expression is compared against each incoming message; when a match occurs, the monitor generates a fault and saves the syslog data.
