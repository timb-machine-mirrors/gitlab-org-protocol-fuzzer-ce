...
* fault detection (fault on a specific state), or 
...
+
[horizontal]
Automation;; Wait for the port to reach a specified state. The _When_ parameter determines when the monitor will wait. The maximum waiting time can be configured by setting the _Timeout_ parameter.
...
OnFault;; Monitoring occurs immediatelly following a fault.
...
===============================
In this example, the call action will cause Peach to wait until the remote port is in an open state. No timeout interval is provided, so Peach will wait forever.
