:images: ../images
<<<
[[Monitors_LinuxDebugger]]
== LinuxDebugger Monitor (Linux)

The _LinuxDebugger_ monitor uses GDB to launch an executable and monitors the executing process for exceptions. 

When a fault or exception occurs, _LinuxDebugger_ collects and logs the appropriate information, including messages from stdout and stderr, stack traces, and the gdb log. Fault detection includes the conditions of 1) the early exit of a process, as well as 2) a process that does not exit. 

_LinuxDebugger_ supports bucketing of crashes, and basic risk ranking that is based on 
...
NOTE: The _LinuxDebugger_ monitor requires a recent version of GDB.
