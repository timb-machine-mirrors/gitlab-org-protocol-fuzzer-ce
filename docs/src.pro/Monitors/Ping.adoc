...
The _Ping_ monitor can verify whether a device is functioning. 

_Ping_ continues to monitor until either a _Timeout_ occurs or it receives a host response. 
...
Also, _Ping_ can verify whether a device has started to function that normally is not functioning. For example, if  you are fuzzing a device that programmatically turns other computers on, you can use _Ping_ with FaultOnSuccess to determine whether the computers are off (uninteresting result) or on (interesting result). 
