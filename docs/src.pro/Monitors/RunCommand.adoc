...
The _RunCommand_ monitor launches a specified command to perform a utility function. The user, through the parameters, can control when the command runs: at the start of the fuzzing session, at the start of an iteration, in the middle of an iteration, after an iteration, and at the end of the session. 
