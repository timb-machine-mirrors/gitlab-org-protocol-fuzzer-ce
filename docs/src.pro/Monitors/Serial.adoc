...
The _serial port_ monitor can be used to perform data collection, fault detection, and automation tasks based on the received input. 

* The default usage of  _serial port_ monitor is data collection. The data received via the serial port is logged when a fault occurs. 
* To perform fault detection, a regular expression can be provided through the FaultRegex parameter. When the regex matches, Peach detects a fault. 
...
WaitWhen:: When to wait. Defaults to OnStart.
