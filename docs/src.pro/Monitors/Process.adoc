 ...
 * Restarts a process when it dies
 * Logs a fault if a process exits early
 * Restarts a process on every iteration
 * Terminates a process if the CPU usage is low
 * Interacts with xref:StateModel[StateModel] to allow waiting for process exit
 * Interacts with xref:StateModel[StateModel] to allow for delayed starting of a process
...
StartOnCall:: Start process on a call action from the xref:StateModel[StateModel] (this value and the method value must match)
WaitForExitOnCall:: Wait for process to exit on state model call (this value and the method value must match)
WaitForExitTimeout:: Wait timeout value.  Triggers fault when timeout hit. Defaults to 10000. Use -1 for infinite, no timeout
...
When running this example, notepad will open when the session starts and close when the session finishes.
