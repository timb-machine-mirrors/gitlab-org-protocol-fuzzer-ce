 ...
 * Interacts with the state model to allow waiting for process exit
 * Interacts with the state model to allow for delayed starting of a process
...
StartOnCall:: Start process on a call action from the state model (this value and the method value must match)
WaitForExitOnCall:: Wait for process to exit on a call from the state model (this value and the method value must match)
