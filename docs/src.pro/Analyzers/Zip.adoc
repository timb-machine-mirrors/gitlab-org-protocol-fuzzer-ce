...
Map:: Controls the stream name to data model mappings.  This parameter is a comma separated list of mappings.  Each mapping is a tuple containing a regular expression match and a DataModel name.  The tuple is delineated with a forward slash character, similar to the substitution regular expression pattern.  When the regular expression matches the file name in the zip archive the corresponding DataModel will be used for the file contents.  For example, to use the data model 'TextModel' for all files ending in '.txt' the parameter's value will be '/.txt$/TextModel/'.
