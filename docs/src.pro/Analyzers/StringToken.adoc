...
The String Token Analyzer creates a tokenized tree from an input string based on special characters and punctuation in the string. For example, the string "?key=value&key2=value" creates a tree similar to the following:
...
When run using the command line, the results of the generated model are saved to disk for custom modification and use. Once saved, you can use and modify the results as needed.
