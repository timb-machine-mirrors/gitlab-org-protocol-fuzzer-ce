...
Peach attempt to fill all three types. You can use data sets to specify which choice to make at different stages in the state model.
...
.An Array of Choices
==========================
The first example is good for making a single choice but what if there are many Type1, Type2, and Type3 blocks following one another? An array is the form to use. To declase the _Choice_ as an array, set the minOccurs, maxOccurs, or occurs atribute to specify that the choice should be repeated.
