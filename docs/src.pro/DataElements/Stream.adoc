...
Stream elements are used to represent multiple blocks of files that contain other files (like zip files) in a single model. Stream elements are similar to xref:Block[Block] elements except they contain file names and file attributes. 

Stream is a child element of either xref:DataModel[DataModel] or xref:Block[Block].

IMPORTANT: xref:Publishers_Zip[Zip] publisher is the only publisher aware of Stream elements. All other publisher will treat Stream elements as if they were Block elements.
...
Streams also include two additional pieces of metadata, a stream name and stream attribute. Stream aware publishers can use the metadata and content to combine all streams in a data model into a single file. 
...
The Stream element is very similar to the [[DataModel_Block]] element except the Stream element includes two pieces of additional data: the stream name and stream attributes.
...
The Stream element is intended for use by publishers that are stream aware. If the publisher is not stream aware, the stream element is treated exactly like a xref:Block[Block].
