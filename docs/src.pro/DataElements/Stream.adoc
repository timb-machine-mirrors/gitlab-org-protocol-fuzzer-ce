...
For example, the [[Publishers_Zip]] takes creates a zip file entry for each corresponding stream element.
If the publisher is not stream aware, the stream element is treated exactly like a [[DataElements_Block]].

=== Examples

.Stream with [[Publishers_Zip]]
==========================
...
.Stream with [[Publishers_ConsoleHex]]
==========================
