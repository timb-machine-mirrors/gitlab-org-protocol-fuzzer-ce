...
Stream elements are used to represent multiple blocks of files that contain other files (like zip files) in a single model. 

Stream elements are similar to xref:Block[Block] elements except they contain file names and file attributes. 

Stream is a child element of either xref:DataModel[DataModel] or xref:Block[Block].

IMPORTANT: Zip is the only publisher aware of Stream elements.
