#TODO Updated defination of a sequence.

<<<
[[Sequence]]
== Sequence

Sequence elements are used to group one or more data elements such as xref:Number[Number] or xref:String[String] together into a logical structure. Sequences can be thought of as arrays that can contain more than a single data type.

They are most useful when used within a Json element as they are used to represent JSON arrays.
