...
xref:Mutators_NumberEdgeCase[NumberEdgeCase]:: This mutator will produce numerical edge cases for integer values.
xref:Mutators_NumberRandom[NumberRandom]:: This mutator will produce random values from the available numerical space.
xref:Mutators_NumberVariance[NumberVariance]:: This mutator will produce values near the current value of a number.
...
*bits*. `Number` elements do not accept `length` attribute used by other elements.
==========================
