...
Blocks are used to group one or more data elements such as xref:Number[Number] or xref:String[String] together into a logical structure. 

Blocks and xref:DataModel[DataModels] are very similar; both can be used as the template for other Blocks or xref:DataModel[DataModels]. Their only difference is their location. 

* DataModels are a top level element 
* Blocks are a child of xref:DataModel[DataModel] 
