...
.Referencing A Block
==========================
A Block can use a reference definition from another container element to form its base definition. In this example, the Block _MyName_ gets its base defintion from the DataModel named _OtherDataModel_. All child elements declared in _MyName_ become part of the block.

NOTE: If a declared child element and a child element from the referenced contain have the same name, the definition of the declared child element is used. In other words, if a naming collision occurs, the declared child element definition overrides the defintion from the referenced container element.
