...
Defines an XML element, the basic building block of XML documents. The _XmlElement_ is used to fuzz the content of an XML document, but not the XML parser.  All the output produced from _XmlElement_ and xref:XmlAttribute[XmlAttribute] will be well formed.
...
xref:minOccurs[minOccurs]:: The minimum number of times this element must occur.
xref:maxOccurs[maxOccurs]:: The maximum number of times this element can occur.
ns:: XML namespace
