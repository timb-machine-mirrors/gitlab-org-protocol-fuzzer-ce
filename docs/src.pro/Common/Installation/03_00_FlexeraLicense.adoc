...
Each test case that Peach executed is considered one 'use' of the Peach license against the total number of test cases purchased.

After submitting a purchase order, Peach will create a user profile that can be used to access the Peach Fuzzer download site.
...
==== Cloud License Server (Online Synchronization)
...
{product} will automatically activate the first time it is run.
...
Ensure both variables are set prior to starting {product}.
