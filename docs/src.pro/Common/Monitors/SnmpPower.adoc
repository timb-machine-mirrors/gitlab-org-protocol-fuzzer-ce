...
The _SNMP Power_ monitor switches outlets on a power distribution unit (PDU) on and off via SNMP.
...
similar features, specifically for IP Power 9258 devices. For
controlling power to a device by wiring through a relay, Peach
provides a monitor for the xref:Monitors_CanaKitRelay[CanaKit 4-Port
...
OIDs::
Comma-separated list of SNMP OIDs for the power outlets.
...
OnCode::
On indicator used by outlet OIDs. Default is `1`, the value commonly used on APC PDUs.

OffCode::
Off indicator used by outlet OIDs. Default is `2`, the value commonly used on APC PDUs.

ReadCommunity::
SNMP community string to use when reading the state of outlet OIDs. Default is `public`.

WriteCommunity::
SNMP community string to use when modifying the state of outlet OIDs. Default is `private`.
...
When::
When to reset power on the specified outlet OIDs. Default is `OnFault`.
+
...
|OnCall                      |
Reset power when the call specified by the 'StartOnCall' parameter is received from the state model.
|==========================================================
+
