...
TIP: Peach also supports APC power distribution units that can be
controlled using SNMP (see xref:Monitors_ApcPower[APC Power
Monitor]). For controlling power to a device by wiring through a
relay, Peach provides a monitor for the
