...
When a fault or exception occurs,
_Gdb_ collects and logs the appropriate information,
including messages from `stdout` and `stderr`,
stack traces,
and the `gdb` log.
Fault detection includes the conditions of 1) the early exit of a process,
as well as 2) a process that does not exit.
