...
Multiple monitors may also be configured to use the same port to allow for fault detection, automation, and/or data collection to occur on a single port.
...
FaultRegex::
Fault when the specified regular expression matches received data.
...
====================
This parameter example is from a setup that uses the default settings for the Serial monitor,
