...
The _APC Power_ monitor switches outlets on a power distribution unit (PDU) on and off via SNMPv1.
...
Outlets::
Comma-separated list of APC outlet numbers to power cycle.
For APC devices that control outlets in groups, configure `OutletGroups` instead.

OutletGroups::
Comma-separated list of APC outlet group numbers to power cycle.
For APC devices that control individual outlets, configure `Outlets` instead.
