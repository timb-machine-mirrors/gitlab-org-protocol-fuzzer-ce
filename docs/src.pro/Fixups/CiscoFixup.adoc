...
The _CiscoFixup_ is a Custom Peach algorithm to fix  Cisco's one-off error in their CRC implementation (known feature).
 ...
 * xref:ref[_ref_] -- Reference to data element which the CRC calculation will be performed on.

=== Supported Algorithms
 
 Internet Checksum (with off-by-one Cisco bug) 
