...
We don't have to worry about the fact the last 3 numbers are offsets.  The data is already parsed in the wave list chunk, we just need to read them in.  So lets build the XML!
...
Lets head down to the _Wav_ chunk. The last time we touched, it looked like this:
