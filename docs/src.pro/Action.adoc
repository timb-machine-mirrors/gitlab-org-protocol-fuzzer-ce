...
* Actions can also control the Peach environment by changing to another state in the xref:StateModel[StateModel], moving data between DataModels, and calling custom methods defined by Agents.

Many Actions are called by Peach implicitly (without explicitly declaring it within a State). Two examples are:

* The start action that performs every iteration automatically  
* The open action called before any outputs or inputs
