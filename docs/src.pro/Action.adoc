...
Action elements are child elements of xref:State[State]. Actions send commands to the xref:Publisher[Publisher].

Action elements perform various operations in the xref:StateModel[StateModel].

* Actions can send output, receive input, open a connection, and much more. For example, the open action initializes a listener by initiating a network connection or opening a file handle. Similarly, the close action releases resources.
* Actions can control the Peach environment by changing to another state in the xref:StateModel[StateModel], moving data between xref:DataModel[DataModels], and calling custom methods defined by xref:Agent[Agents].

Many Actions are called by Peach implicitly (without an explicit declaration in a State). Two examples follow:

* The xref:Action_start[_start_] action performs every iteration automatically  
* The  xref:Action_open[_open_] action called before any outputs or inputs occur.
...
xref:Action_onComplete[onComplete]:: Expression to run when an action completes
