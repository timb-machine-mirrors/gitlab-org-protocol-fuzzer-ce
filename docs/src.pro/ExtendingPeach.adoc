...
{product} was designed to be extended, allowing the end user increase the capabilities of Peach in new ways. The most common reasons for extending Peach include: custom validation algorithms such as checksums, custom publishers (I/O adapters), and monitoring. However, Peach also includes the ability to write custom agents, loggers, strategies, analyzers, and strategies. This section will cover the most common types of extensions to Peach.
...
All plugins take a C# dictionary as a construction argument.
This dictionary of arguments is populated with the names and values of the Param elements defined in the pit.
This example shows the arguments in the constructor of the JsEncode transformer.
...
Peach plugins define all of their supported configuration arguments by decorating the plugin class with the ParameterAttribute.
The ParameterAttribute defines the name, type, description and optional default value of each plugin parameter.
The name and type specified in the ParameterAttribute must match a public property on the plugin class.
If the name parameter happens to be a C# reserved keyword, the associated class property is prefixed with an underscore.
If the default value is omitted from the ParameterAttribute, Peach treats the parameter as required.
Lastly, the default value is always provided as a string, regardless of the type of the parameter.
...
The ParameterParser.Parse() method is used to automatically populate all plugin properties from the dictionary of arguments.
 ...
 * Throw PeachException() when non-recoverable errors occur
 ** Parameter is out of range
 ** User lacks appropriate permissions
 * Throw SoftException when recoverable errors occur
 ** File in use
