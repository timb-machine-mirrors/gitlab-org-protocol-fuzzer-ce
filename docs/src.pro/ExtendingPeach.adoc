...
Since their values are based on other elements, fixups track whenever a referenced element changes so they can recompute their value. For example, the Crc32 fixup needs to know whenever the referenced element changes, so that a new crc can be computed. 
