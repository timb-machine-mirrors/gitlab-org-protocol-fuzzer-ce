...
If you have custom/proprietary targets, you may need to extend Peach's functionality by using plugins. 

Peach uses custom implementations of xref:Fixup[Fixups], xref:Transformer[Transformers], xref:Publisher[Publishers],and xref:Monitors[Monitors]Monitors to extend its functionality.
...
--
When Peach runs, it will automatically reflect all assemblies to get the list of xref:Fixup[Fixups], xref:Transformer[Transformers], xref:Publisher[Publishers],and xref:Monitors[Monitors]Monitors.
