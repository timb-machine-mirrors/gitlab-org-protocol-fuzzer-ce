...
The _close_ Action will cause the Publisher to close it's resource.
For the xref:Publishers_File[File Publisher], this will close the file handle.
For the xref:Publishers_Tcp[TCP Publisher] it will close the network connection.
Because the _close_ Action will occur by default at the end of each iteration, the only time the _close_ Action needs to be explicitly called is when the default behaviour needs to be changed.
A common example of explicitly calling the _close_ Action is during file fuzzing. During file fuzzing the data:
will be written using an xref:Action_output[_output_] Action 
then closed using a _close_ Action 
and finally the target application launched using a xref:Action_call[_call_] Action
...
==========================
This is an example of the default implicit behaviour of _close_.
...
==========================
This is an example of needed to call _close_ in a different order than default.
