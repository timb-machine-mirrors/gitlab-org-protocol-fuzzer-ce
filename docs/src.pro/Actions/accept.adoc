...
The _accept_ Action will block execution until an incoming connection is available, made, and accepted, or a time out is reached.
This action is most common in situations where a target application initiates communication and Peach is playing the roll of a server. 
Not all xref:Publisher[Publishers] support this action type. One that does support the _accept_ action is the xref:Publishers_TcpListener[TcpListener] Publisher.
Peach is single-threaded so it is unable to execute any scripts or other tasks while waiting for a connection from the target.
Some Publishers use an AcceptTimeout parameter for setting a duration of time to wait for a connection.
...
xref:Iteration_control[control iteration]::
  Peach will trigger a fault. Note that control iterations must be enabled for this state to be reached.
  On control iterations it is assumed that any error is the result of the target entering an unwanted state.
  Such transitions are considered faults and logged as such.
  
xref:Iteration_fuzzing[fuzzing iteration]::
  Peach will continue to next iteration.
  During normal fuzzing iterations it is assumed we will cause odd behaviour from the target which should be ignored unless they occur on a control iteration (an iteration with out any mutations occurring).
****
