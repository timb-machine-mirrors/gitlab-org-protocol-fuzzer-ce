...
The _open_ (or _connect_) action causes the associated publisher to open its resource. 
The _open_ action is an implicit Peach action that, in general, should not be declared 
in the xref:StateModel[StateModel]. The _open_ Action occurs at the start of each iteration.
...
The time to declare the _open_ action in the StateModel is when the default behavior needs to change. For example, 

//The only time you need to explicitly call the _open_ action is when the default behavior needs to be changed (for example, when listening for an initial incoming UDP packet because unlike the //xref:Action_accept[_accept_] action used with TCP, UDP does not block while accepting incoming packets).
