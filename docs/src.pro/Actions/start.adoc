...
*NOTE:* Including the _start_ or _stop_ action in the StateModel will cause the action to run every iteration instead of once per session. This declaration should be avoided unless specifically required.
...
==========================
Example Pit showing explicit declaration of the _start_ and _stop_ actions.
The _start_ and _stop_ declaractions cause them to be called every iteration, as seen in the output listing.
