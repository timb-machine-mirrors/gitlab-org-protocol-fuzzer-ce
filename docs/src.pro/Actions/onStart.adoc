...
The onStart attribute is an expression that is evaluated before preforming an action. 
This expression can be used to increment a counter or preform other functions (like the ones defined in separate files included with the xref:Import[Import] tag).

NOTE: If the _onStart_ expression is on the very first explicit action, it will get executed prior to the implicit actions.
...
==========================
This example will print a message when the _onStart_ expression is run.
