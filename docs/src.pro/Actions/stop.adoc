...
The stop action causes a xref:Publishers[Publisher] to perform any final clean that would otherwise occur at the end of a fuzzing session.
This is one of the implicit actions that will be called by Peach and does not need to be included in the xref:StateModel unless there is a change in the default behaviour.
...
Not all xref:Publishers make use of start/stop.
