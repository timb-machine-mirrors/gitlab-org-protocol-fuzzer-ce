...
*NOTE:* Including the _stop_ or _start_ action in the StateModel will cause the action to run every iteration instead of once per session. This declaration should be avoided unless specifically required.
...
Output showing the default behaviour of the sample pit with implicit usagae for _start_ and _stop_. The _start_ action is called once at the beginning of the fuzzing session and the _stop_ action is called once at the end of the fuzzing session.
