...
The _getProperty_ action retrieves the value of a property from an object that uses the property metaphor and stores the value in a DataModel. _getProperty_ actions specify a xref:DataModel[DataModel] to receive the retrieved data as a child element.
...
* when fuzzing targets such as Microsoft COM/DCOM objects that use the property metaphor.
* when accessing properties in a Publisher that supports the property metaphor. Some Publishers (such as the UDP Publisher) support using properties to get or set values. An instance of this type of Publisher is in the examples section.
