...
The _output_ action is used to write/send in data using a xref:Publishers[Publisher].
...
The exact behaviour of an _output_ action is Publisher dependent. Review the documentation for the specific Publisher for the details on specific implementation actions. For example, calling output multiple times in a row on a stream publisher such as xref:Publishers_Tcp[TCP] will appear as one output, whereas multiple outputs in the xref:Publishers_Udp[UDP] publisher will send the data as separate UDP Packets.
