...
The Slurp action is limited to copying values only between two non-container elements (a container element is one which can contain other elements such as xref:DataElements_Choice[Choice], xref:DataElement_Block[Block], or xref:DataElement_DataModel[DataModel]). Attempting to Slurp a container element will cause a Peach error.
