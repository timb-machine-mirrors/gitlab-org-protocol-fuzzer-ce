...
REST JSON::
	Agent protocol using REST style calls over HTTP. This channel is best suited for custom Peach agents. For more information, see the _Peach Professional Developer Guide_.

TCP:: 
	The Peach agent communicates using REST api calls over TCP. Peach can run as a TCP remoting agent using the +-a tcp+ argument.
...
==== Legacy (formerly TCP Remoting Agent)
