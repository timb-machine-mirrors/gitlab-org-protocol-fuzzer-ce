...
Local::
	Agent is hosted in the current Peach process

TCP Remoting::
	Peach agent communicating using .NET TCP Remoting (RPC) protocol. Requires agent be written using Microsoft.NET framework. Peach can be run as a TCP remoting agent using the +-a tcp+ arguments.
