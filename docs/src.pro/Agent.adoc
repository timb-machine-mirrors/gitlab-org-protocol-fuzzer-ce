...
TCP Remoting::
	Peach agent communicates using .NET TCP Remoting (RPC) protocol. Requires agent be written using Microsoft.NET framework. Peach can be run as a TCP remoting agent using the +-a tcp+ arguments.
...
There is a bug in the Microsoft.NET remoting framework that causes default network adapters IP address to be used, event when communication occurs over a secondary interface.

When running a Peach Agent on a machine that has muliple network interfaces, the interface can be explicitly bound to using a configuration setting. This configuration setting is only required for the Peach Agent process.
