...
Local::
	Agent is hosted in the current Peach process. This is the default channel.

TCP Remoting::
	Peach agent communicates using .NET TCP Remoting (RPC) protocol. The agent must be written using the Microsoft.NET framework. Peach can run as a TCP remoting agent using the +-a tcp+ arguments.
...
The peach runtime supports a local agent that runs in process.  Local Agent is the default agent unless another agent type is specified.
...
To use a remote agent, the agent process must be running first.
...
There is a bug in the Microsoft.NET remoting framework that causes the default IP address of the network adapter to be used, event when communication occurs over a secondary interface.
