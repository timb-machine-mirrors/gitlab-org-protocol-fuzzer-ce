...
When importing Python files into the Peach name space, define both the xref:PythonPath[PythonPath] and xref:Import[Import] elements. The _PythonPath_ element defines where to find the Python files. The _Import_ element specifies the file to import; the .py filename extention is not used in the declaration, as it is implied. Multiple files can be imported.
...
Scripts running in Peach have can access all DataModels, StateModels and Tests.  Data is accessed by traversing the DOM to locate the target element.
...
DefaultValue:: This property allows you to access the default, unmutated value. This is the value produced when this element is not being fuzzed.

InternalValue:: This property contains the value (fuzzed or default) to use when generating the final value. The field type is a variant of the internal typing, such as "int" or "string". the field is used during scripting. 

Value:: This is the final binary value that will be used. This is the InternalValue packed into binary form. For a _Number_ element of size 32, this is 4 contiguous bytes of data with correct endian-ness.

The data and state models in Peach are tree object graphs with a parent child relationship. Parents are accessible using the _parent_ property.

=== Returning Peach Types

When a script overwrites the _DefaultValue_ for any element, the value type must be a Peach Variant. A Peach Variant can accept integers, strings, and arrays.
...
You can debug Python code loaded from a file by using the InteractiveConsole from the code module. When the function is executed and hits the +code.InteractiveConsole+ line, control over the function is transferred to the user until the function returns. This allows each line of code that follows to be manually entered to ensure the expected behavior is happening.
