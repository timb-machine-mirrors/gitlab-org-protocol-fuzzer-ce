...
The _RawV6_ publisher allows sending raw IPv6 packets without IP header. When receiving input, the IPv6 header is included in the received data. The maximum and minimum MTU size of the publisher can be fuzzed by using the publisher's _setProperty_ action.
 ...
 * _Protocol_ -- IP protocol to use (value must be between 0 and 255)
