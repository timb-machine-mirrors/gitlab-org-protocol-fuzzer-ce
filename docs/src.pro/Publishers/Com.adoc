...
NOTE: This Publisher only runs on Windows. To use this Publisher, Peach.Core.ComContainer.exe must be running in a separate command prompt while the Peach is running. This side process will host the COM object. A debugger monitor can be configured to launch and monitor this process if wanted. See xref:Monitors_WindowsDebugger monitor for more information about configuring a debugger monitor.
