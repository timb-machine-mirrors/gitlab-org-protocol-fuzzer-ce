...
image::{images}/linux_tux.png["Linux Tux mascot", scale="40"]
...
. Start the virtual machine snapshot containing the test target.
. Fuzz the device and look for faults.
. When a fault occurs, do the following:
.. Collect data.
.. Revert to a snapshot of the system.
. Loop to step 4 and continue fuzzing.
...
. Select a pit (test definition) of a protocol supported by the network device, so 
...
image::{images}/Local_Agent.png["Local Agent for Network Device", scale="40"]
...
image::{images}/NetworkCapture_VMWare2.png["Network Capture VMWare", scale="50"]
...
. If you haven't already done so, save the Monitor configuration defined so far.
. Add a new agent, and give the agent a name, such as `Remote`. 
. Since this agent will reside in the virtual machine, the location providing the tcp port address 
is appropriate. Then, click the`Save` button.

image::{images}/Remote_Agent_VMSetup.png["Remote Agent for Network Device", scale="40"]
...
image::{images}/VM_Agents_Both.png["Test Configuration With Remote Aagent", scale="50"]
