...
) ENGINE=InnoDB AUTO_INCREMENT=37964 DEFAULT CHARSET=utf8 PACK_KEYS=1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
BEGIN
delete from metrics_iterations;

delete from metrics_faults;

delete from metrics_states;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getpreviousjobid`(
in jobid VARCHAR(12),
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `jobs_insert`(
in jobid VARCHAR(12),
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `metrics_faults_insert`(
jobs_id INT(11),
bucket varchar(100),
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `metrics_iterations_insert`(
in jobs_id INT(11),
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `metrics_states_insert`(
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectbuckettrend`(
in jobid VARCHAR(12)
)
BEGIN
SELECT j.target, j.startdate
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectfaulttrend`(
in jobid VARCHAR(12)
)
BEGIN
SELECT j.target, j.startdate
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectmetricsbydataset`(
in jobid varchar(12)
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectmetricsbyelement`(
in jobid varchar(12)
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectmetricsbymutator`(
in jobid varchar(12)
)
BEGIN
/*
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectstatemetrics`(
in jobid varchar(12)
)
BEGIN
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `viewmetricsbybucket` AS select `j`.`jobid` AS `jobid`,`j`.`startdate` AS `startdate`,`f`.`bucket` AS `bucket`,`f`.`mutator` AS `mutator`,`f`.`state` AS `state`,`f`.`actionname` AS `actionname`,`f`.`parameter` AS `parameter`,`f`.`dataelement` AS `dataelement`,sum(`i`.`iterationcount`) AS `iterationcount`,count(distinct `f`.`metrics_faults_id`) AS `faultcount` from ((`jobs` `j` join `metrics_faults` `f` on((`f`.`jobs_id` = `j`.`jobs_id`))) left join `metrics_iterations` `i` on(((`i`.`jobs_id` = `j`.`jobs_id`) and (`i`.`mutator` = `f`.`mutator`) and (`i`.`state` = `f`.`state`) and (`i`.`actionname` = `f`.`actionname`) and (`i`.`parameter` = `f`.`parameter`) and (`i`.`dataelement` = `f`.`dataelement`)))) group by `j`.`jobid`,`j`.`startdate`,`f`.`bucket`,`f`.`mutator`,`f`.`state`,`f`.`actionname`,`f`.`parameter`,`f`.`dataelement` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `viewmetricsbydataset` AS select `j`.`jobid` AS `jobid`,`j`.`startdate` AS `startdate`,`i`.`dataset` AS `dataset`,sum(`i`.`iterationcount`) AS `iterationcount`,count(distinct `f`.`bucket`) AS `bucketcount`,count(distinct `f`.`metrics_faults_id`) AS `faultcount` from ((`jobs` `j` join `metrics_iterations` `i` on((`i`.`jobs_id` = `j`.`jobs_id`))) left join `metrics_faults` `f` on(((`f`.`jobs_id` = `j`.`jobs_id`) and (`f`.`dataelement` = `i`.`dataelement`)))) group by `j`.`jobid`,`j`.`startdate`,`i`.`dataset` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `viewmetricsbyelement` AS select `j`.`jobid` AS `jobid`,`j`.`startdate` AS `startdate`,`i`.`dataelement` AS `dataelement`,count(distinct `i`.`mutator`) AS `mutatorcount`,sum(`i`.`iterationcount`) AS `iterationcount`,count(distinct `f`.`bucket`) AS `bucketcount`,count(distinct `f`.`metrics_faults_id`) AS `faultcount` from ((`jobs` `j` join `metrics_iterations` `i` on((`i`.`jobs_id` = `j`.`jobs_id`))) left join `metrics_faults` `f` on(((`f`.`jobs_id` = `j`.`jobs_id`) and (`f`.`dataelement` = `i`.`dataelement`)))) group by `j`.`jobid`,`j`.`startdate`,`i`.`dataelement` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `viewmetricsbymutator` AS select `j`.`jobid` AS `jobid`,`j`.`startdate` AS `startdate`,`i`.`mutator` AS `mutator`,count(distinct `i`.`dataelement`) AS `dataelementcount`,sum(`i`.`iterationcount`) AS `iterationcount`,count(distinct `f`.`bucket`) AS `bucketcount`,count(distinct `f`.`metrics_faults_id`) AS `faultcount` from ((`jobs` `j` join `metrics_iterations` `i` on((`i`.`jobs_id` = `j`.`jobs_id`))) left join `metrics_faults` `f` on(((`f`.`jobs_id` = `j`.`jobs_id`) and (`f`.`mutator` = `i`.`mutator`)))) group by `j`.`jobid`,`j`.`startdate`,`i`.`mutator` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2013-12-10 12:22:10
