...
def generateNewSymbols(newData, symbolStartPos, main_literals, main_distances):
	'''
	Use the huffman tables to generate new data to
	inject or replace the current value.
	'''
	
	newSymbol = None
	for i in range(20):
		newR = random.randint(258, 284)
		for l in main_literals.table:
			if l.code == newR:
				newSymbol = l.symbol
				break
	...
	e1 = extra_length_bits(newR)
	e1Value = random.randint(0, 2 ** e1)
	e2 = extra_length_bits(newR1)
	e2Value = random.randint(0, 2 ** e2)
	
	if random.choice([True, False]):
		newData.seek()
	
	newData.writebits(newSymbol, newBits)
	newData.writebits(e1Value, e1)
	newData.writebits(newSymbol1, newBits1)
	newData.writebits(e2Value, e2)
...
def gzip_main(data):
	...
	# Modify Bitfield to take data instead
	b = Bitfield(field)
	out = ''
	...
	totalMutations = random.randint(0, 7)
	mutationCount = 0
	...
	while True:
		...
		if blocktype == 0:
			...
			if length & b.readbits(16):
				...
				print "Error, stored block lengths do not match each other"
				return
			...
			while True:
				...
				# Will we mutate this one?
				mutateThisKey = random.choice([False,False,False,True])
				...
				if 0 <= r <= 255:
					...
					if mutateThisKey:
						sizeOfConstruct = b.tellbits() - lz_start
						newSymbol = None
						for i in range(20):
							newR = random.randint(258, 284)
							for l in main_literals.table:
						...
						if newSymbol != None:
							...
							if newSymbol1 != None:
								...
								newBits = newSymbol + e1Value
								newBits += newSymbol1 + e2Value
