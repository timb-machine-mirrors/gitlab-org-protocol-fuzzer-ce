...
namespace PitMaker.Models
{
  ...
  public class DataElement : NodeWithParameters, IXmlSerializable
  {
    public DataElement() { }

    public DataElement(Type dataElementType)
      :base(dataElementType)
    {
      if (dataElementType.IsSubclassOf(typeof(Peach.Core.Dom.DataElement)) == false)
      {
        throw new Peach.Core.PeachException(dataElementType.Name + " is not a valid Data Element type");
      }


      this.PeachType = dataElementType;
    }
    ...
    [Browsable(false)]
    [System.Xml.Serialization.XmlElementAttribute("Analyzer", typeof(Analyzer))]
    ...
    public void ReadXml(System.Xml.XmlReader reader)
    {
      ...
      List<Type> dataElementTypes = (from Type t in Peach.Core.ClassLoader.GetAllTypesByAttribute<Peach.Core.Dom.DataElementAttribute>(null) where t.IsSubclassOf(typeof(Peach.Core.Dom.DataElement)) && t.IsSubclassOf(typeof(Peach.Core.Dom.DataElementContainer)) == false select t).ToList();
      ...
      if (this.PeachType != null)
      {
        ...
        /*
        var result = (from XAttribute a in xmlattributes where a.Name.LocalName == "value" select a).FirstOrDefault();
        if (result != null)
          this.Value = result.Value;


        
        result = (from XAttribute a in xmlattributes where a.Name.LocalName == "valueType" select a).FirstOrDefault();
        if (result != null)
        {
          this.ValueType = Utilities.ParseFromXml<Peach.Core.Dom.ValueType>(result.Value);
          //this.ValueType = (Peach.Core.Dom.ValueType)Enum.Parse(typeof(Peach.Core.Dom.ValueType), result.Value);
        }
        //*/
          ...
          case "Relation":
            switch (child.Attribute("type").Value)
            {
              ...
              case "offset":
                ...
                break;
              default:
                throw new Peach.Core.PeachException(child.Name.LocalName + " is not a valid child of " + element.Name.LocalName);
            }
            break;
          case "Transformer":
