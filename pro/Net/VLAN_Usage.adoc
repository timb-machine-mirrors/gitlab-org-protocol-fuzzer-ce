...
First we must install vlan and configure vlan. Once that is done socat needs to be set up in different terminals to listen for incoming packets. For this example we assume you are running linux. For other platforms follow the platform specific installation instructions for vlan and socat.
