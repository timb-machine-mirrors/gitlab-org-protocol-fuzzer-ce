...
using Peach.Pro.OS.Windows.Debuggers.DebugEngine;
...
namespace Peach.Pro.OS.Windows.Agent.Monitors
{
	...
	public class WindowsKernelDebugger : Monitor2
	{
		...
		KernelDebuggerInstance _debugger;
		...
		private void LaunchDebugger()
		{
			...
			_debugger.Run(KernelConnectionString);
			// Start side process
			// Get debugger ready to accept connections
		}
		...
		public class KernelDebuggerInstance : MarshalByRefObject, IDisposable
		{
			static readonly NLog.Logger Logger = LogManager.GetCurrentClassLogger();

			public class WinDbg : IDisposable, IDebugEventCallbacks, IDebugOutputCallbacks
			{
				...
				public bool HandlingException { get; private set; }
				...
				{
					Logger.Trace("LoadModule: {0} {1} {2} {3} {4} {5}", ImageFileHandle, BaseOffset, ModuleSize, ModuleName, ImageName);
					return 0;
				}
					...
					lock (_thread)
					{
						...
						if (_winDbg != null)
						{
							...
							else
							{
								...
								_winDbg.DebugClient.EndSession(DEBUG_END.END_REENTRANT);
							}
						}
					}
			...
			public void Run(string kernelConnectionString)
			{
				if (_thread != null)
